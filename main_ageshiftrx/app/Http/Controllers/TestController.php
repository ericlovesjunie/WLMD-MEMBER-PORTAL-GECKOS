...
public function getSingleProductPrice(Request $request)
    {
        $product = $this->getSingleProductPrice_api($request->email, $request->product_id);
        return response()->json([
            'status'  => 1,
            'message' => "Great! You have Successfully get all record.",
            'product' => $product
        ]);
    }

private function getSingleProductPrice_api($email, $product_id)
    {
...
public function get_weight_loss_products_v2(Request $request)
    {
        $email = $request->email;

        // 🔹 Step 1: Prepare mapping (sticky_product_id => order_total)
        $productOrderMap = [];
        ...
        if (!empty($email)) {
            ...
            if ($app_key_data) {
                ...
                foreach ($site_data as $site) {
                    ...
                    // ❌ Remove status = 7
                    $orders = array_filter($responseData['data'], function ($order) {
                        return $order['order_status'] !== "7";
                    });

                    if (empty($orders)) continue;

                    // 🔹 Latest order (by date if available)
                    usort($orders, function ($a, $b) {
                        return strtotime($b['time_stamp'] ?? '') - strtotime($a['time_stamp'] ?? '');
                    });
                    ...
                    foreach ($latestOrder['products'] as $product) {
                        ...
                        // Mapping store
                        $productOrderMap[$stickyProductId] = $order_total;
                    }
                }
            }
        }
        ...
        if ($table_data->count() != 0) {
            ...
            foreach ($table_data as $data) {
                ...
                // 🔥 Replace price if order exists
                $price = $data->product_price;

                if (isset($productOrderMap[$data->sticky_product_id])) {
                    $price = $productOrderMap[$data->sticky_product_id];
                }
	...
	public function get_weight_loss_products(Request $request)
          {
              $table_data = DB::table('product')->where('is_member','1')->where('status',1)->get();
