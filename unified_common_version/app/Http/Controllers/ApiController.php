...
class ApiController extends Controller
{
	...
	public function user_logout(Request $request)
    {
        ...
        // Signal AttachAuthCookies to clear login cookies
        session(['logout_cookies' => true]);
        session()->flush();
	...
	public function getOrderOpTest(Request $request, $site_id, $order_id)
    {
        ...
        if (
            ...
            $apiEmail = $responseData['data']['email'] ?? '';

            // Email match check
            if (strtolower(trim($apiEmail)) === strtolower(trim($email))) {
        ...
        // Original API error return
        return response()->json($responseData, $response->status());
    }

    public function user_logout237(Request $request)
    {
        $email = session('user_email') ?? $request->email;
    ...
    public function verify_verification_code(Request $request)
    {
        ...
        if($user)
        {
            ...
            if($user->is_confirmed == 0 && $user->expiration_time > now())
            {
             ...
             $this->setUserSession($user, $login_token);
 ...
 public function create_login_tokens($email)
    ...
    public function get_product_details_v3(Request $request)
    {
        ...
        if($product_sku!="")
        {
            $is_db = DB::table('product')->where('product_sku',$product_sku)->first();
                    ...
                    else
                    {
                        ...
                        $gender  = $user_tb->gender == 1 ? "Male" : "Female";
                            if ($user_tb->date_of_birth) {
                                ...
                                $this->setUserSession($user_tb, $login_token);
                                // $result['login_token'] = $login_token;
                            $result['patientId']    = "";
                                $result['customer_id']   = $customer_id;
                                // $result['existing']     = 1;
...
private function setUserSession($user, $login_token)
    {
        session([
            ...
            try {
                ...
                DB::table('campaign')->updateOrInsert(
                    ...
                    [
                        ...
                        'offer_id' => json_encode($data['offer_id'] ?? []),
...
// dd($product_data);
    ...
    public function check_coupons(Request $request)
    {
        ...
        if (empty($promo_code) || empty($amount)) {
            return response()->json([
                'status' => 0,
                'message' => 'Please fill out all the required details in the form before submitting it.'
            ]);
   ...
   public function get_instance_pixel(Request $request)
    ...
    public function subscription_stop(Request $request)
    {
        ...
        foreach ($products as $product) {
            $subscription_id = $product['subscription_id'] ?? null;
            ...
            if (!empty($subscription_id)) {
                ...
                if ($response->successful()) {
                    ...
                    $failedSubscriptions[] = [
                        'subscription_id' => $subscription_id,
                        'error' => $responseData['message'] ?? 'Failed to stop subscription'
                    ];
                }
            }
        }
        ...
        return response()->json([
            'status' => 1,
            'message' => 'Subscription stop process completed.',
            'stopped' => $stoppedSubscriptions,
    ...
    public function offer_view(Request $request)
    {
        ...
        $response = Http::withBasicAuth($app_key, $api_ext)
            ->post('https://whitelabelmd.sticky.io/api/v1/offer_view', $payload);
        ...
        if ($response->successful()) {
            ...
            // Check if trial_flag is 1
            $trial_flag = $offerData['trial_flag'] == "1";
    
            // Check if 'products' key exists and is an array
            if (isset($offerData['products']) && is_array($offerData['products'])) {
                foreach ($offerData['products'] as $productId => $productName) {
                    DB::table('product')->updateOrInsert(
                        ['sticky_product_id' => $productId],
                        [
                            // 'trial_flag' => $trial_flag ? 1 : 0,
                            'offer_id' => $offer_id,
                            // 'product_name' => $productName, // Save the product name
                            // 'custom_price' => $trial_flag ? ($offerData['trial']['price']['custom_price'] ?? 0) : 0,
                            'updated_at' => now(),
                        ]
                    );
                }
            ...
            return response()->json([
                'status' => 0,
                'message' => 'Failed to get offer data.',
                'error' => $response->json(),
            ], $response->status());
        }
    }
